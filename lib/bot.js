...
class Bot {
  constructor(options) {
    ...
    this.ircSelfHighlight = Object.assign({ enabled: true, modifier: '\u200B' }, options.ircSelfHighlight); // default to true
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (!this.ircSelfHighlight.enabled) {
        displayUsername = `${displayUsername.slice(0, 1)}${this.ircSelfHighlight.modifier}${displayUsername.slice(1)}`;
      }
  ...
  sendToDiscord(author, channel, text) {
    ...
    // Remove zero-width (/u200B is getting converted into â€‹ incorrectly)
    const withFormat = formatFromIRCToDiscord(text.replace('â€‹', ''));
